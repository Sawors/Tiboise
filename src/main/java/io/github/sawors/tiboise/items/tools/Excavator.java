...
public class Excavator extends RadiusBreakingTool implements Listener {
    ...
    public static void onPlayerDestroyBlock(BlockBreakEvent event){
        ...
        if(target.isValidTool(item) && TiboiseItem.getItemId(item).equals(new Excavator().getId())){
    ...
    public @Nullable ShapedRecipe getRecipe(ItemVariant variant) {
        ...
        ShapedRecipe recipe = new ShapedRecipe(new NamespacedKey(Tiboise.getPlugin(),variant.toString().toLowerCase(Locale.ROOT)+"_"+getId()),new Excavator(variant).get());
        recipe.shape(
                "MMM",
                "MSM",
                "XSX"
